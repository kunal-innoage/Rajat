    ...
    def get_product_image_id(self, image_url, product):
        ...
        if response:
            if response.status_code == 200:
                ...
                if data:
                    product_image_id = self.env['product.image'].create({'image_1920':data, 'name': product.default_code+ ' Flat'})
                    # _logger.info("~~~~Rajat18~~~~~~~~ %r ~~~~~~~~~~~~~~", product_image_id)
                    return product_image_id
        return False
    
    def get_product_image_id1(self, image_url, product):
            ...
            if rec.sku:
                ...
                for product in products:
                    if product.flat_image:
                        ...
                        try:
                            product_image_added = self.get_product_image_id1(product.flat_image, product)
                            if product_image_added:
                    ...
                    if product.corner:
                       ...
                       # Check for existing corner image in this product 
                        ...
                        if to_delete_product_image:
                            to_delete_product_image.unlink()
